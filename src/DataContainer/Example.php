...
#[AsCallback(table: 'tl_example', target: 'edit.buttons', priority: 100)]
class Example
{
    ...
    public function __invoke(array $arrButtons, DataContainer $dc): array
    {
        ...
        $systemAdapter->loadLanguageFile('tl_exsample');

        if ('edit' === $inputAdapter->get('act')) {
            $arrButtons['customButton'] = '<button type="submit" name="customButton" id="customButton" class="tl_submit customButton" accesskey="x">'.$GLOBALS['TL_LANG']['tl_exsample']['customButton'].'</button>';
        }
